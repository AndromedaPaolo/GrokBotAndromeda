...
**Proposta:** `Fantasy_Empire_Proposta_Commerciale.md` v2.5
...
|---|---|---|
| **Cursor** | Repo, PR, test, flag, pagine, schema D1, checkout in preview | Inbox giocatori, X, ads, invio mail, job Imagine |
| **GrokBot** | Mail, X, ricerca web/norme, Grok Imagine, bozze social | Merge, combat, Price ID, `STRIPE_LIVE`, file in R2 (li copia il Worker) |
...
**Otto su Cursor, nove su GrokBot.** Ads (G9) non sta dentro Bacheca: un post e un'inserzione a pagamento non sono lo stesso tasto.
...
| C8 | Stagione | panchina | `git_pr` (tipo spento) | Season e cosmetic. Niente pay-to-win. Si siede quando c'è margine *e* tu sblocchi. |
...
| G4 | Imagine | acceso | `video_new` | Still + I2V. Worker copia su R2. Quota giocatore la scala il Worker. |
| G5 | Corriere | acceso, tasto condizionato | `preavviso_pagamenti`, poi mail transazionali Visioni | Preavviso 30 giorni. In Fase B: "abbonamento attivo / disdetto / pagamento fallito". |
| G6 | Sportello | panchina | `mail` tag `soldi` | "Ho pagato e il Santuario è chiuso", chargeback, rimborso. Bozza. Non tocca i fondi. |
| G7 | Promo | panchina | `mail_massa` | Newsletter e patch notes marketing. Solo consenso. Niente in Fase 0. |
| G8 | Bacheca | panchina | `post_x` / `post_ig` / `post_altro` | Publish social. Disclosure IA. Riusa cache `cinematics`. |
| G9 | Spesa | panchina | `ads` | Copy e budget. Il click sulla carta ads resta tuo. Mai in Fase 0. |

---

## 4. Chi gira già, chi sta seduto

**Accesi o in preview, Fase 0.** Patcher, Sito, Numeri, Checkout, Santuario, Bandiere, Verbale, Inbox, Ascolto, Gazzetta, Imagine, Corriere (il preavviso: card sì, Approva spento se la checklist è rossa).
...
Job xAI. Chiave del progetto. Mai una chiave del giocatore. Precache o tetto. Il combat non aspetta. File: Worker → R2 → `video_new`. Approva = `ready`. Scarta = delete.

`gen_quota` non è affare suo. La decrementa il Worker quando il job produce un file nuovo.
...
Imagine finisce una clip
  → Worker R2 → video_new → tu Approvi/Scarti
  → C1 non c'entra, salvo bug del player (allora Patcher)

tu vuoi i pagamenti
  → C7 Verbale verde
  → G5 Corriere manda i 30 giorni (Approva tuo)
  → 30 giorni
  → C6 Bandiere, card stripe_live (Approva tuo)
  → C4 Checkout live, C5 Santuario si apre per gli abbonati
  → G6 Sportello esce dalla panchina
```

Vietato il salto: GrokBot che mergea. Cursor che manda mail. Un agent che fa il giro Gazzetta+PR+merge da solo. Un cron "è il 1, si paga".
