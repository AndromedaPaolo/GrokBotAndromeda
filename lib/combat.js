...
/** Status on the opponent. They do not stack. Blind and Bound are spent; Dazed and Wound last the fight. */
export const STATUSES = {
  blind: {
    id: "blind",
    name: "Blind",
    it: "Blind",
    hint: "The next action misses",
  },
  bound: {
    id: "bound",
    name: "Bound",
    it: "Bound",
    hint: "Skip the next turn",
  },
  dazed: {
    id: "dazed",
    name: "Dazed",
...
export const CATEGORIES = {
  normal: {
    ...
    name: "Normal",
    chance: 0.2,
    where: "A character's starting cards",
  },
  fusion: {
    id: "fusion",
    name: "Fusion",
  ...
  origin: {
    ...
    name: "Origin",
    chance: 0.8,
    where: "A monster's birth",
  },
  unique: {
    id: "unique",
    name: "Unique",
    chance: 1,
    where: "Invasion super bosses",
  },
};
...
/**
 * Chance to apply status/drain (not damage, not Bound).
 * Normal, and Fusion of two Normal cards: 20%. Bond/Special/Origin: 80%. Unique: 100%.
 * Mixed Fusion: uses the higher parent category.
 */
...
/** Bound (skip the turn): Bond rarely, Unique always. Every other category: 0. */
export const SKIP_CHANCE = {
  bond: 0.15,
  unique: 1,
};

export function skipChance(card, catalog = []) {
  return skipChanceFrom(card, catalog, new Set());
}

function skipChanceFrom(card, catalog, seen) {
  const cat = cardCategory(card);
  if (cat === "unique") return SKIP_CHANCE.unique;
  if (cat === "bond") return SKIP_CHANCE.bond;
  if (cat !== "fusion") return 0;

  const id = `skip:${card.id ?? JSON.stringify(card.fusionOf ?? [])}`;
  if (seen.has(id)) return 0;
  seen.add(id);

  const parents = parentCards(card, catalog);
  if (!parents.length) return 0;
  return Math.max(...parents.map((parent) => skipChanceFrom(parent, catalog, seen)));
}

export function landingStatus(card, catalog = []) {
  if (!card?.status) return null;
  if (card.status === "bound" && skipChance(card, catalog) <= 0) return null;
  return card.status;
}

export function cardEffectChance(card, catalog = []) {
  if (landingStatus(card, catalog) === "bound") return skipChance(card, catalog);
  return effectChance(card, catalog);
}
...
export const EFFECT_COPY = {
  dazed: {
    name: "Dazed",
    doesEn: "they act last in the turn order",
    doesIt: "they act last in the turn order",
  },
  wound: {
    name: "Wound",
    doesEn: "they take +1 damage from every hit",
    doesIt: "they take +1 damage from every hit",
  },
  blind: {
    name: "Blind",
    doesEn: "their next action misses",
    doesIt: "their next action misses",
  },
  bound: {
    name: "Bound",
    doesEn: "they skip their next turn",
    doesIt: "they skip their next turn",
  },
};

export function cardFlavor(card) {
  const flavor = String(card?.flavor || "").trim();
  if (flavor) return flavor.replace(/\.+$/, "");
  const text = String(card?.text || "").trim();
  const cut = text.search(/\. Applies /);
  if (cut >= 0) return text.slice(0, cut).trim();
  const dot = text.indexOf(".");
  return (dot >= 0 ? text.slice(0, dot) : text).trim();
}

export function appliedEffect(card, catalog = []) {
  const chance = cardEffectChance(card, catalog);
  const pct = Math.round(Number(chance) * 100);
  const status = landingStatus(card, catalog);
  if (status && EFFECT_COPY[status]) {
    const copy = EFFECT_COPY[status];
    return {
      kind: status,
      name: copy.name,
      pct,
  ...
  if (drain > 0) {
    return {
      ...
      name: "Drain",
      pct,
      doesEn: `they lose ${drain} AP`,
      doesIt: `they lose ${drain} AP`,
    };
  }
  return null;
}

export function cardAppliesLine(card, catalog = []) {
  const fx = appliedEffect(card, catalog);
  if (!fx) return "";
  return `Applies ${fx.name} (${fx.pct}%): ${fx.doesEn}.`;
}

export function effectBadge(card, catalog = []) {
  const fx = appliedEffect(card, catalog);
  if (!fx) return "";
  return `${fx.name} ${fx.pct}%`;
}

export function cardRulesText(card, catalog = []) {
  const flavor = cardFlavor(card);
  const applies = cardAppliesLine(card, catalog);
  const cost = Math.max(0, Number(card?.sp) || 0);
  return [flavor ? `${flavor}.` : "", applies, `Cost is damage (${cost}).`]
    .filter(Boolean)
    .join(" ");
}

export function withCardRules(card, catalog = []) {
  return { ...card, text: cardRulesText(card, catalog) };
}

/** Base AP = assigned cards ÷ 6. */
...
/** Draw 6. A pool of 6 is a permutation. A smaller pool draws with replacement. */
...
/**
 * One card, not the whole bar. If leftover AP is possible, often keep it.
 * If every card would empty the bar, sometimes pass and hold AP.
 */
...
function playCard(state, actor, card, rng = Math.random) {
  ...
  const chance = cardEffectChance(card, catalog);
  const drain = Math.max(0, Number(card.drainAp) || 0);
  const hasFx = Boolean(status) || drain > 0;
  let damage = 0;
  let drained = 0;
  const applied = [];
  let effectHit = false;
  ...
  if (!missed && foe) {
    ...
    if (effectHit) {
      if (addStatus(foe, status)) applied.push(status);
      if (drain) {
  ...
  state.recentCards = [...(state.recentCards ?? []), card].slice(-3);
}
...
/**
 * Pass without playing. Leftover AP stays.
 * Does nothing on the monster: it plays at random, Skip is off.
 */
...
/**
 * Advance one action. No timer: the still stays until you call again.
 * One card per turn; the next click ends the turn and keeps leftover AP.
 * Monster: randomly plays a card or passes to keep AP.
 * Bound: the turn is skipped. Blind: the action misses, no effect on the opponent.
 */
...
/** Who still needs to play: the current actor, or the next in order. */
export function nextToAct(state) {
  const actor = currentActor(state);
  if (!actor) return null;
  const stillToPlay = (actor.actionsThisTurn || 0) === 0 && !hasStatus(actor, "bound");
  if (stillToPlay) return actor;
  const nextId = (state.order ?? []).find((id) => id !== actor.id && !(state.actedIds ?? []).includes(id));
  if (nextId) return state.units.find((u) => u.id === nextId) ?? null;
  return state.units.find((u) => u.id !== actor.id) ?? null;
}
